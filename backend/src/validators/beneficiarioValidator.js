...
export const criarBeneficiarioSchema = z.object({
  ...
  // Utilizado para calcular automaticamente a quantidade de cestas.
  composicaoFamiliar: z
    .number({
      required_error: "Composição familiar é obrigatória.",
      invalid_type_error: "Composição familiar deve ser um número.",
    })
    .int("Composição familiar deve ser um número inteiro.")
    .min(1, "Composição familiar deve ser maior que zero."),
  ...
  tipoBeneficio: z.enum(["CESTA", "GRANEL", "AMBOS"]),
  ...
  instituicaoId: z.number().int().positive().optional(),
});
