...
const register = async (req, res) => {
  try {
    const { nome, email, senha, senhaHash, role, instituicaoId } = req.body;
    ...
    if (
      ...
      dadosNovoUsuario.instituicaoId = Number(instituicaoId);
    }
...
const login = async (req, res) => {
  try {
    ...
    if (!usuario.senhaHash) {
      console.error(`Usuário de ID ${usuario.id} está sem senhaHash no banco.`);
